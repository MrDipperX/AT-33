...
from config.config import DBNAME, HOST, USER, PASSWORD, PORT
...
class PgConn:
        ...
        with self.conn:
            ...
            if id_data is None:
                ...
                for subject in subjects:
                    self.cur.execute(sql.SQL("INSERT INTO {subject}(id_user) "
                                             "VALUES((SELECT id FROM users WHERE id_tg = %s));")
                                     .format(subject=sql.Identifier(subject)), (user_id,))
                    self.conn.commit()
            else:
                return
            return id_data
    ...
    def set_task_status(self, user_id, subject, task, status):
        with self.conn:
            self.cur.execute(sql.SQL("UPDATE {subject} SET {task} = %s "
                                     "WHERE id_user = (SELECT id FROM users WHERE id_tg = %s);")
                             .format(subject=sql.Identifier(subject), task=sql.Identifier(task)), (status, user_id))
            self.conn.commit()

    def get_task_status(self, user_id, subject, task):
        with self.conn:
            self.cur.execute(sql.SQL("SELECT {task} FROM {subject} "
                                     "WHERE id_user = (SELECT id FROM users WHERE id_tg = %s);")
                             .format(subject=sql.Identifier(subject), task=sql.Identifier(task)), (user_id,))
